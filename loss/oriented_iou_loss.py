import torch
import numpy as np
from loss.min_enclosing_box import smallest_bounding_box
from loss.box_intersection_2d import oriented_box_intersection_2d
...
def cal_giou_3d(box3d1:torch.Tensor, box3d2:torch.Tensor, enclosing_type:str="smallest"):
    ...
    """
    assert torch.all(box3d1[:,:,3:6]>=0)
    iou3d, corners1, corners2, z_range, u3d,iou_2d = cal_iou_3d(box3d1, box3d2, verbose=True)
    w, h = enclosing_box(corners1, corners2, enclosing_type)
    v_c = z_range * w * h
    giou_loss = 1. - iou3d + (v_c - u3d)/v_c
    # giou_loss = 1-iou3d
    return giou_loss, iou3d,iou_2d
